...
public struct OrderedDictionary<Key: Hashable, Value>: RandomAccessCollection, MutableCollection {
    ...
    public init() {
        ...
        self.init(uniqueKeysWithValues: EmptyCollection<Element>(), minimumCapacity: nil)
    }
    ...
    private init<S: Sequence>(uniqueKeysWithValues keysAndValues: S, minimumCapacity: Int?) where S.Element == Element {
    ...
    internal mutating func _sort(in range: Range<Index>, by areInIncreasingOrder: (Element, Element) throws -> Bool) rethrows {
    ...
    internal mutating func _partition(in range: Range<Index>, by belongsInSecondPartition: (Element) throws -> Bool) rethrows -> Index {
